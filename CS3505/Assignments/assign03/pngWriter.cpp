...
PNGWriter::~PNGWriter() {
  ...
  // We need to deallocate the two dynamically allocated variables
...
void PNGWriter::saveToFile(char *fileName) {
  ...
  // We need to initialize the cache struct for the PNG.
  png_structp png =
  ...
  // Simple check to ensure creation of the two structs suceeded.
  if (!info || !png) {
